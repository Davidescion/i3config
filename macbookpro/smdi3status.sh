...
CPU=$(printf "%.*f\n" 2 $floatCPU)
floatKB=$(cat /sys/class/leds/smc\:\:kbd_backlight/brightness | awk '{usage=($1/255)*100} END {print usage}')
KB=$(printf "%.*f\n" 2 $floatKB)
lol=$(echo "Keyboard:" $KB "%" "|" "CPU:" $CPU "% |" "Used Memory:" $mem "MB" "|" "Kernel:" $kernelver "|" "Time:" $date)
echo -n $lol
sleep 1
done